...
function PostContainer() {
  ...
  const [careerAge, setCaeerAge] = useState("");
  ...
  const postValidationCheck = () => {
    ...
    if (!title) {
      dispatch({type: "OPEN", payload: {title:"제목을 입력해주세요.", content: ""}});
      return false;
    }
    if (!inputs.content) {
      dispatch({ type: "OPEN", payload: { title: "질문이 비었어요!", content: "최소 한글자 이상 입력해주세요." } });
      return false;
    }
    if (!primaryJobCategory) {
      dispatch({ type: "OPEN", payload: { title: "직종이 선택되지 않았어요!", content: "직종을 선택해주세요." } });
      return false;
    }
    return true;
  };
